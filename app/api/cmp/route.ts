...
export async function GET(request: NextRequest) {
  ...
  if (symbols.length > 30) {
    return NextResponse.json(
      { error: "Maximum 30 symbols per request" },
      { status: 400 }
    );
  }

  // ✅ FIX: Convert string[] → {symbol, exchange}[]
  const stockObjects = symbols.map((s) => ({
    symbol: s,
    exchange: "NSE", // or "BSE" or auto-detect if needed
  }));

  try {
    const { value: cmpData, isStale } = await fetchWithCache(
      `cmp:${symbols.sort().join(",")}`,
      () => getCMPBatch(stockObjects) // <-- NOW CORRECT
    );
